...
# The fallback unnecessarily converts coefficients to Float64
function Base.literal_pow(f::typeof(^), r::Complex{T}, ::Val{p}) where {p, T<:QuadraticRing}
    Base.power_by_squaring(r, p)
end
...
function Base.:*(::RootTwoT, r::Union{Rational, Dyadic})
    d = Dyadic(r)
    DRoot2(zero(d), d)
end
Base.:*(r::Union{Rational, Dyadic}, ::RootTwoT) = RootTwo * r
