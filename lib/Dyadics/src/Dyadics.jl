...
import RingExtensionsUtils: superscript, iszero_strong, isone_strong, greater_than_strong,
    PRETTY, lobit
import RingExtensionsCommon: canonical, mul_half, mul_two, conj_root_two, isunit, invstrict
import SingletonNumbers: InvTwo, InvTwoT, TwoT, Pow

using RingExtensionsCommon: RingExtensionsCommon, params
...
```
"""
RingExtensionsCommon.params(d::Dyadic) = (d.a, d.k)
...
function promote_rule(::Type{Dyadic{T1,V1}}, ::Type{T}) where {T1<:Base.BitInteger, V1<:Integer, T<:BigFloat}
    BigFloat
end
...
function promote_rule(::Type{Dyadics.Dyadic{T1, V1}}, ::Type{T}) where {T1<:BigInt, V1<:Integer, T<:BigFloat}
    BigFloat
end
